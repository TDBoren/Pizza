...
function getOther(runningTotal,text1) {
    ...
    for (var l = 0; l < otherArray.length; l++) {
        if (otherArray[l].checked) {
            selectedOther.push(otherArray[l].value);
            console.log("selected other item: ("+otherArray[l].value+")");
            text1 = text1+otherArray[l].value+"<br>";
        }
    }
    var otherCount = selectedOther.length;
    if (otherCount > 1) {
        otherTotal = (otherCount - 1);
    } else {
        otherTotal = 0;
    }
    runningTotal = (runningTotal + otherTotal);
    console.log("total selected other items: "+otherCount);
    console.log(otherCount+" other - 1 free other = "+"$"+otherTotal+".00");
    console.log("other text1"+text1);
    console.log("Purchase Total: "+"$"+runningTotal+".00");
    document.getElementById("showText").innerHTML=text1;
    document.getElementById("totalPrice").innerHTML = "<h3>Total: <strong>$"+runningTotal+".00"+"</strong></h3>";
}
